...
As elaborated by @messick89, the validity argument involves a claim with
...
### 3.1A Alignment Between AA-AAAS and Academic Content Standards {.unnumbered}
...
### 3.1B AA-AAAS Linkage to General Content Standards {.unnumbered}
...
## Validity Based on Cognitive Processes
...
## Validity Based on Internal Structure (Content and Function)
...
### 3.3A Point Measure Correlations {.unnumbered}

```{r}
# code for point measure correlation here tables here
# Use it to make in-text references below respond to the data
```
...
visualizations were conducted with **ggplot2** in the **tidyverse** package [@wickham19].
...
```{r}
# code for mean square outfit tables here (mean, min, max)
```

### 3.3B Annual Measureable Objectives Frequencies & Percentages {.unnumbered}
...
conducted with *ggplot2* in the *tidyverse* suite of packages [@wickham19].

```{r}
# code for tables reporting the percentage of students in each performance level (1-4)

# We also produced bar plots of these in the past
```
...
### 3.4A Convergent and Divergent Validity Documentation {.unnumbered}
...
### 3.4B Analyses Within and Across Subject Areas {.unnumbered}
...
### 3.4C Correlational Analyses Results {.unnumbered}
...
```{r}
# code for between-content correlations
```
